...
// Template variable is actually a reference to the child component or the component
// that is being used in the html. Or in which component declaration the variable is defined
// property binding -> square braces is for outputting or spitting a property to child component []
@Component({
    ...
    <div>
      ...
      <div class="row">
        <div class="col-md-5" *ngFor="let myEvent of events">
          <event-thumbnail [event]="myEvent"></event-thumbnail>
<!--          <event-thumbnail (childEvent)="handleChildEvent($event)" [event]="event1" #thumbnailComponent></event-thumbnail>-->
        </div>
      </div>

<!--      <h3>{{thumbnailComponent.someProperty}}</h3>-->
<!--      <button class="btn btn-primary" (click)="thumbnailComponent.logFoo()">Log me some foo</button>-->
    </div>
  `
})
